...
def get_stats(d,flist,sample):
	...
	for file in flist:
		...
		for line in fh:
			ct_sample+=1
			if d['pass']==1 and (t < 1) and (ct_sample %  (4*sample)  != 1):
				continue
			entry.append(line)
			t+=1
			if t==4:
...
if __name__ == '__main__':
	...
	parser.add_argument('--sample_fq', default=10000, help='skip missing file output.')
	...
	get_stats(stats,files1,int(args.sample_fq))
	get_stats(stats,files2,0)
